...
async def async_setup_entry(
    ...
    async_add_entities([
        ...
        ISEEVYVerifyChannelButton(coordinator, entry),
    ])
...
class ISEEVYVerifyChannelButton(CoordinatorEntity, ButtonEntity):
    """Button entity to poll the decoder for the REAL current channel via netstat."""

    def __init__(self, coordinator: ISEEVYDataUpdateCoordinator, entry: ConfigEntry) -> None:
        super().__init__(coordinator)
        self._entry = entry
        self._attr_unique_id = f"{entry.entry_id}_verify_channel"
        self._attr_name = "Verify Real Channel"
    ...
    async def async_press(self) -> None:
        """Press the button - poll real channel via netstat (ground truth)."""
        await self.coordinator.async_verify_channel()
